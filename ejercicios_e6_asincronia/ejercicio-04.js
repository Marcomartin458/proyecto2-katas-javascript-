...
const mayoresDeEdad = ages1.filter(age => age > 18);
...
const legendsStreamers = streamers43.filter(streamer => streamer.gameMorePlayed === 'League of Legends');
...
const uStreamers = streamers44.filter(streamer => streamer.name.includes('u'));
...
const nuevoStreamers = streamers44.filter(streamer => streamer.gameMorePlayed.includes('Legends'))
  .map(streamer => {
    if (streamer.age > 35) {
